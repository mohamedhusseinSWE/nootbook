...
import { Library, Plus, Search, BookOpen, FileText, Trash2, Edit3, FolderPlus, Tag } from "lucide-react";
...
const LibraryPage: React.FC = () => {
  ...
  const [isUploadingToTopic, setIsUploadingToTopic] = useState(false);
  ...
  const deleteTopic = async (topicId: string) => {
    if (!confirm("Are you sure you want to delete this topic and all its notes?")) return;
  ...
  const handleTopicFilesUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <BannedUserProtection>
      {/* Top header */}
      <Header title="Library" subtitle="Organize and combine your notes by topic" />

      {/* Fixed sidebar */}
      <MainSidebar sidebarOpen={sidebarOpen} onToggleSidebar={() => setSidebarOpen((s) => !s)} />

      {/* Main content area with left margin to accommodate sidebar */}
      <div className={`min-h-screen bg-gray-50 ${sidebarOpen ? "ml-64" : "ml-16"}`}>
                  ...
                  {topics.map(topic => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-500">{topic.noteCount} notes</div>
                      </button>
                      <button
              ...
              {selectedTopic && (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Attach multiple files (PDF, DOC/DOCX, TXT, MD) under this topic
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-center gap-3">
                      <span className="text-sm text-gray-700">Topic:</span>
                      <span className="text-sm font-medium">{topics.find(t => t.id === selectedTopic)?.name}</span>
                    </div>
                    <div className="mt-3">
